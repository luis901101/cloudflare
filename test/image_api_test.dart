...
void main() async {
  ...
  test('Handling image from url tests', () {
    ...
    final image6 = CloudflareImage(
      id: 'm3xgriuradsz3ed23',
      imageDeliveryId: '4m5x3gt2o5htuergn',
    );
  ...
  group('Upload image tests', () {
    test(
      'Simple upload image from file with progress update',
      () async {
    ...
    test(
      'Simple upload image from bytes with progress update',
      () async {
        if (!imageFile.existsSync()) {
    ...
    test(
      'Simple upload image from url with progress update',
      () async {
        if (imageUrl.isEmpty) {
    ...
    test(
      'Multiple upload image from file with progress update',
      () async {
        if (!imageFile.existsSync() ||
    ...
    test(
      'Multiple upload image from url with progress update',
      () async {
        if (imageUrl.isEmpty) {
          fail(
    ...
    test(
      'Upload image with requireSignedURLs and metadata',
      () async {
        if (!imageFile.existsSync()) {
          fail('No image file available to upload');
        }
    ...
    group('Image direct upload tests', () {
      ...
      test(
        'Doing image upload to direct upload URL',
        () async {
