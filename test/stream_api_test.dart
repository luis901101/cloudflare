...
void main() async {
  ...
  group('Stream video tests', () {
    test(
      'Simple stream video from file with progress update',
      () async {
    ...
    test(
      'Simple stream video from bytes with progress update',
      () async {
        if (!videoFile.existsSync()) {
    ...
    test(
      'Simple stream video from url with progress update',
      () async {
        if (videoUrl.isEmpty) {
    ...
    test(
      'Multiple stream video from file with progress update',
      () async {
        if (!videoFile.existsSync() ||
    ...
    test(
      'Multiple stream video from url with progress update',
      () async {
        if (videoUrl.isEmpty) {
          fail(
    ...
    test(
      'Stream video from url with requireSignedURLs',
      () async {
        if (!videoFile.existsSync()) {
          fail('No video file available to stream');
        }
    ...
    group('Video direct stream tests', () {
      ...
      setUpAll(() async {
        ...
        response = await cloudflare.streamAPI.createDirectStreamUpload(
      ...
      test(
        'Check created video ready to stream status',
        () async {
          if (dataUploadDraft?.id.isEmpty ?? true) {
            fail('No videoId available to check video status');
          }
          final response = await cloudflare.streamAPI.get(
            id: dataUploadDraft?.id,
          );
          expect(response, StreamVideoMatcher());
          expect(
            response.body?.readyToStream,
            false,
      ...
      test(
        'Doing video stream to direct stream URL',
        () async {
  ...
  group('tus protocol stream upload tests', () {
    ...
    test(
      'Doing authenticated stream upload using tus protocol',
      () async {
        if (!videoFile.existsSync()) {
          fail('No video file available to stream');
        }
        ...
        final testProgressCallback = expectAsyncUntil2(
          onProgress,
          () => isComplete,
        );
        try {
    ...
    test(
      'Create authenticated direct stream video URL using tus protocol',
      () async {
    ...
    test(
      'Doing direct stream upload using tus protocol',
      () async {
        if (dataUploadDraft?.uploadURL.isEmpty ?? true) {
          fail('No streamURL available to stream to');
        }
  ...
  test('Delete video test', () async {
    ...
    final response = await cloudflare.streamAPI.delete(id: videoId);
    expect(response, ResponseMatcher(), reason: response.error?.toString());
